...
def main():
    ...
    ap.add_argument("--out_dir", type=str, default="Outputs/Sesion01/knn_binary_sesion01_top30")
    ...
    for pid, path in sorted(files.items()):
        ...
        # ================================================================
        # === ENTRENAR KNN LIMPIO PARA SHAP (KernelSHAP) =================
        # ================================================================
        import joblib
        from sklearn.neighbors import KNeighborsClassifier
        ...
        clean_params = {k.replace("knn__", ""): v for k, v in gs.best_params_.items()}
        ...
        # KNN limpio
        knn_clean = KNeighborsClassifier(**clean_params)
        knn_clean.fit(Xtr_scaled, ytr)

        # Guardar scaler + modelo
        joblib.dump(knn_clean, os.path.join(args.out_dir, f"knn_clean_for_shap_participant_{pid}.pkl"))
        joblib.dump(scaler_clean, os.path.join(args.out_dir, f"scaler_knn_for_shap_participant_{pid}.pkl"))

        print(f"[SAVE] KNN limpio SHAP para participante {pid}")
        # ================================================================


        # Evaluación local con mejores hiperparámetros
    ...
    for pid, info in per_participant.items():
        ...
        # ============================================================
        # === ENTRENAR KNN GLOBAL LIMPIO PARA SHAP ====================
        # ============================================================
        from sklearn.neighbors import KNeighborsClassifier
        from sklearn.preprocessing import StandardScaler
        import joblib

        # Extraer hiperparámetros globales sin prefijo "knn__"
        clean_global_params = {
            k.replace("knn__", ""): v
        ...
        # Crear modelo limpio (sin pipeline)
        knn_clean_global = KNeighborsClassifier(**clean_global_params)

        # Entrenar con datos escalados
        knn_clean_global.fit(Xtr_scaled_global, ytr)

        # Guardar modelo + scaler para SHAP
        joblib.dump(knn_clean_global, os.path.join(args.out_dir, f"knn_global_clean_for_shap_participant_{pid}.pkl"))
        joblib.dump(scaler_global, os.path.join(args.out_dir, f"scaler_knn_global_for_shap_participant_{pid}.pkl"))
        ...
        rows_global.append({
            ...
            "oob_global": None,  # KNN no tiene OOB
    ...
    # CSV resumen
    summary_path = os.path.join(args.out_dir, "knn_trialwise_binary_trust_summary.csv")
    ...
    summary_global_path = os.path.join(args.out_dir, "knn_global_summary_binary_trust.csv")
    ...
    best_json_path = os.path.join(args.out_dir, "best_global_params_binary_trust_knn.json")
