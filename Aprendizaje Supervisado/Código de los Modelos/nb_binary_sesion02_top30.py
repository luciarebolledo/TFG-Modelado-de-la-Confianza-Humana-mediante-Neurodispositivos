...
def main():
    ...
    ap.add_argument("--out_dir", type=str, default="Outputs/Sesion02/nb_binary_sesion02_top30")
    ...
    # Grid NB
    param_grid = {
        "nb__var_smoothing": [1e-12, 1e-10, 1e-8, 1e-6, 1e-5]
    }
    ...
    for pid, path in sorted(files.items()):
        ...
        print(f"[BEST L0CAL HYPERPARAMETERS]: {gs.best_estimator_}")

        # ================================================================
        # === ENTRENAR NAIVE BAYES LIMPIO PARA SHAP ======================
        # ================================================================
        import joblib
        from sklearn.naive_bayes import GaussianNB

        # No hay prefijos → los hiperparámetros van directos
        clean_params = {k.replace("nb__", ""): v for k, v in gs.best_params_.items()}

        # NB limpio (sin escalado)
        nb_clean = GaussianNB(**clean_params)
        nb_clean.fit(Xtr, ytr)

        # Guardar modelo SHAP
        joblib.dump(nb_clean, os.path.join(args.out_dir, f"nb_clean_for_shap_participant_{pid}.pkl"))

        print(f"[SAVE] Naive Bayes limpio SHAP para participante {pid}")
        # ================================================================
    ...
    for pid, info in per_participant.items():
        ...
        # ============================================================
        # === ENTRENAR NAIVE BAYES GLOBAL LIMPIO PARA SHAP ============
        # ============================================================
        from sklearn.naive_bayes import GaussianNB
        import joblib

        # NB no usa hiperparámetros del GridSearch → se entrena directo
        nb_clean_global = GaussianNB()

        # Entrenar con Xtr, ytr del participante
        nb_clean_global.fit(Xtr, ytr)

        # Guardar modelo GLOBAL limpio para SHAP
        global_model_path = os.path.join(
            args.out_dir,
            f"nb_global_clean_for_shap_participant_{pid}.pkl"
        )
        joblib.dump(nb_clean_global, global_model_path)

        # === Guardar en tabla de resultados globales ===
        rows_global.append({
            "participant": pid,
            ...
            "global_te_f1": te_ev["f1"],  # NB usa F1 normal
            "oob_global": None,  # NB no tiene OOB
            "global_params": "{}"  # NB no tiene hiperparámetros
        })
        # ============================================================
    ...
    # CSV resumen
    summary_path = os.path.join(args.out_dir, "nb_trialwise_binary_trust_summary.csv")
    summary.to_csv(summary_path, index=False)

    # ============================================
    # CSV GLOBAL (igual que hiciste en RF)
    # ===========================================
    summary_global = pd.DataFrame(rows_global).sort_values("participant")
    summary_global_path = os.path.join(args.out_dir, "nb_global_summary_binary_trust.csv")
    ...
    best_json_path = os.path.join(args.out_dir, "best_global_params_binary_trust_nb.json")
